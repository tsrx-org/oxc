...
`results-1789846886123.json`. The ordinary `oxfmt` median is 35.61 ms
versus canonical Oxfmt 0.68.0's 31.82 ms on the identical files; p95 is 36.93
ms versus 32.89 ms (1.123×). Exact normalized stdout/stderr and exit status
match, and trace evidence records zero TSRX dispatch events. The mixed
companion p95 is 65.69 ms for lint (1.926× canonical two-file TSX) and 66.57
ms for format-check (2.024× canonical; the mixed lane formats an unformatted
`.tsrx` file, which the settling pass formats twice). A complete Vite+ 0.2.4
mixed lint is 296.54 ms p95. The
