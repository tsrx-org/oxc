  ...
  try {
    ...
    await writeFile(
      ...
      `${JSON.stringify({
        ...
        private: true,
        type: "module",
        devDependencies: { "@tsrx/oxc": "0.11.0" },
      }, null, 2)}\n`,
    );
...
test("the canonical command names arbitrate from the project manifest alone", async () => {
  ...
  try {
    ...
    const plain = await project("plain", { "@tsrx/oxc": "0.11.0" });
    ...
    const transitive = await project("transitive", { "@tsrx/oxc": "0.11.0" }, { oxlint: officialOxlint });
    ...
    // tool.
    const missing = await project("missing", { "@tsrx/oxc": "0.11.0", oxfmt: "0.44.0" });
...
async function providerFixture(temporary, name, { ownsLinterShim }) {
  ...
  await writeFile(
    ...
    `${JSON.stringify({ name: "@tsrx/oxc", version: "0.11.0", bin: { oxlint: "./bin/oxlint" } }, null, 2)}\n`,
  );
