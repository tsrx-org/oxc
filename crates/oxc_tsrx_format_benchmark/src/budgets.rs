...
pub(crate) fn validate_budgets(budgets: &Budgets) -> Result<(), String> {
    ...
    // parsed once more to decide whether a second pass is needed, and every benchmark corpus is
    // unformatted, so the direct-route ratio against the single-pass canonical control moved
    // from 1.05/1.08 to 1.25/1.35 (observed 1.15/1.26) and the generalized-control floors from
    // 15/12 MiB/s to 12/10 MiB/s (observed 16.1/15.0). Clean input still costs one pass.
    if budgets.p04.direct_median_latency_ratio_max > 1.25
        || budgets.p04.direct_p95_latency_ratio_max > 1.35
        ...
        || budgets.p04.generalized_control_median_mib_per_second_min < 12.0
        || budgets.p04.generalized_control_p95_mib_per_second_min < 10.0
